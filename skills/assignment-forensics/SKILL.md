...
- Requirement conflicts or ambiguous instructions, such as tests that exist but must be ignored, incomplete work still requiring PR submission, library freedom versus familiar-method guidance, React Query being available but not clearly required, or multiple APIs that overlap in responsibility.
...
Extract rules that affect correctness: validation, ordering, pricing, selection, payment/order state, permissions, filtering, sorting, data freshness, or retry behavior.
